...
#Gender Percentage
Gender <- c(sample('Male',round(769*.46),replace = T),
            sample('Female',round(769*.54),replace = T))
...
Income <- c(sample(50000,round(769*.53),replace = T),
            ...
            sample(100001:300000,round(769*.15),replace = T),
            sample(0,round(769*.04),replace = T))
...
Geographic_Area <- as.data.frame(Geographic_Area)
colnames(Geographic_Area) <- 'Area_Type'
Geographic_Area$id <- NA
Geographic_Area <- Geographic_Area %>%
  mutate(across(c(id), ~ 
                  case_when(Area_Type == 'Rural' ~  '1', Area_Type  == 'Suburban' ~ '2', Area_Type == 'Urban'  ~ '3'))) %>%
  relocate(id, Area_Type)

#Create data frame
Data <- as.data.frame(cbind(Gender,Generation,Employment_Status,Region,Children,Income))
Data$Geo_id <- Geographic_Area$id[sample(1:nrow(Geographic_Area))]
Geographic_Area <- Geographic_Area %>% 
  distinct(id,Area_Type)
Data$Income <- as.numeric(Data$Income)

rm(Gender,Generation,Employment_Status,Region,Children,Income)
...
to.remove <- ls()
to.remove <- to.remove[-which(to.remove == 'Data' | to.remove == 'Geographic_Area')]
rm(list=to.remove)
rm(to.remove)
